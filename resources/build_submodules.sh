...
clangversion=$(clang++ --version | grep 10.0) >/dev/null 2>&1
...
if [ $? -ne 0 ]; then
  bash set_instrumenter_directory.sh $extsourcedir/llvm-instrumentation/build/lib >/dev/null 2>&1 

  # TODO We should check whether a cube / scorep install was found and if so, bail out, to know exactly which scorep/cube we get.
  aclocalversion=$( aclocal --version | grep 1.13 ) >/dev/null 2>&1 
  if [ -z "$aclocalversion" ]; then
    echo "aclocal available in wrong version. Guessing to autoreconf."
    autoreconf -ivf >/dev/null 2>&1 
  	cd ./vendor/cubelib >/dev/null 2>&1 
  	autoreconf -ivf >/dev/null 2>&1 
  	cd ./build-frontend >/dev/null 2>&1 
  	autoreconf -ivf >/dev/null 2>&1 
  	cd ../../cubew >/dev/null 2>&1 
  	autoreconf -ivf >/dev/null 2>&1 
  	cd ./build-backend >/dev/null 2>&1 
  	autoreconf -ivf >/dev/null 2>&1 
  	cd ../build-frontend  >/dev/null 2>&1 
  	autoreconf -ivf >/dev/null 2>&1 
  	cd $extsourcedir/scorep-mod
  ...
  ../configure --prefix=$extinstalldir/scorep --enable-shared --disable-static --disable-gcc-plugin "$add_flags"  >/dev/null 2>&1 
  if [ $? -ne 0 ]; then
...
python -m pip install --user PyQt5 >/dev/null 2>&1
if [ $? -ne 0 ]; then
  echo "[PIRA] Installting Extra-P dependency PyQt5 failed."
  exit 1
fi

python -m pip install --user matplotlib >/dev/null 2>&1
if [ $? -ne 0 ]; then
...
if [ ! -f "extrap-3.0.tar.gz" ]; then
  ...
  wget http://apps.fz-juelich.de/scalasca/releases/extra-p/extrap-3.0.tar.gz >/dev/null 2>&1
  tar xzf extrap-3.0.tar.gz >/dev/null 2>&1
fi
cd ./extrap-3.0
...
if [ $? -ne 0 ]; then
  ...
  command -v locate "/Python.h" >/dev/null 2>&1
  if [ $? -eq 1 ]; then
...
if [ ! -d "$extsourcedir/cxxopts" ]; then
    git clone https://github.com/jarro2783/cxxopts cxxopts >/dev/null 2>&1 
fi
cd cxxopts
echo "[PIRA] Select release branch 2_1 for cxxopts."
git checkout 2_1 >/dev/null 2>&1 

# JSON library
echo "[PIRA] Getting json library"
cd $extsourcedir
if [ ! -d "$extsourcedir/json" ]; then
    git clone https://github.com/nlohmann/json json >/dev/null 2>&1 
		cd json
		git checkout v3.7.3 >/dev/null 2>&1 
fi
...
if [ $? -ne 0 ]; then
  rm -rf build
  mkdir build && cd build
  cmake -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$extinstalldir/cgcollector -DJSON_INCLUDE_PATH=$extsourcedir/json/single_include .. >/dev/null 2>&1 
  if [ $? -ne 0 ]; then
...
if [ $? -ne 0 ]; then
  rm -r mpiwrap >/dev/null 2>&1
  mkdir mpiwrap && cd mpiwrap
  wget https://github.com/LLNL/wrap/archive/master.zip >/dev/null 2>&1
  unzip master.zip >/dev/null 2>&1
  rm -rf $extinstalldir/mpiwrap >/dev/null 2>&1
