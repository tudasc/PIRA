...
#Check build config parameters
  ...
  for items in configuration.items[key]:
    ...
    if (util.check_provided_directory(configuration.items[key][items]['builders']) == False):
      ...
      exit(1)
    assert (
        configuration.items[key][items]['builders'] == '/home/sachin/IdeaProjects/master/benchpress/functors2'
        or
    ...
    if (configuration.items[key][items]['submitter'] == ''):
      ...
      if (util.check_provided_directory(configuration.items[key][items]['runner']) == False):
        logger.log("Runner functor directory is invalid." + configuration.items[key][items]['runner'],
                   'error')
        exit(1)
      assert (
          configuration.items[key][items]['runner'] == '/home/sachin/IdeaProjects/master/benchpress/functors2'
          or
      ...
      if (util.check_provided_directory(configuration.items[key][items]['submitter']) == False):
        logger.log("Submitter functor directory is invalid." + configuration.items[key][items]['submitter'],
                   'error')
        exit(1)
    ...
    if (util.check_provided_directory(configuration.items[key][items]['instrument_analysis'][0]) == False):
      logger.log(
          "Instrument Analysis functor directory is invalid." +
          configuration.items[key][items]['instrument_analysis'][0], 'error')
      exit(1)
    assert (configuration.items[key][items]['instrument_analysis'][0] ==
            '/home/sachin/IdeaProjects/master/benchpress/functors2' or configuration.items[key][items]
            ['instrument_analysis'][0] == '/home/sachin/IdeaProjects/master/benchpress/functors')

    assert (
        configuration.items[key][items]['instrument_analysis'][1] ==
        '/home/sachin/MasterThesis/lulesh203/lulesh' or
    ...
    if (util.check_provided_directory(configuration.items[key][items]['instrument_analysis'][2]) == False):
      logger.log(
          "Instrument Analysis tool directory is invalid." +
          configuration.items[key][items]['instrument_analysis'][2], 'error')
      exit(1)
...
for globalFlavor in configuration.global_flavors:
  ...
  if (util.check_provided_directory(configuration.global_submitter[globalFlavor]) == False):
    logger.log(
        "Global Submitter directory is invalid." + configuration.items[key][items]['instrument_analysis'][2],
        'error')
    exit(1)
  assert (
      configuration.global_submitter[globalFlavor] == '/home/sachin/IdeaProjects/master/benchpress/functors2'
      or
